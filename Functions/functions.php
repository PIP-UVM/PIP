<?php
    function showChoices($questionName, $choices, $disabled = true, $userAnswer = null) {
        ...
        for ($choice = 0; $choice <= 4; $choice++) {
          ...
          if (!$disabled) {
            if ($checked) {
              print "<input type = radio value = '" . $choice . "' required name = '" . $questionName . "' checked>";
            } else {
              print "<input type = radio value = '" . $choice . "' required name = '" . $questionName . "'>";
            }
    ...
    function sendInvitation($record_id, $email) {
      $surveyURL = "http://vip.w3.uvm.edu/DEVELOP/survey.php?ur=" . $record_id . "&section=collabagree";
			...
			$bcc = "";
			$from = "vip@uvm.edu";
			$subject = "PIP Survey Link";
			$message = "<p>This is the link to your Practice Integration Profile survey.</p>";
