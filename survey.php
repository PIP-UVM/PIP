...
$results = $thisDatabase->select($query, $data);
		...
		elseif(isset($_POST['btnNext'])) {
			...
			// Because 'ident' section doesn't use the same naming scheme for percents
			if ($sectionId != 'ident') {
			...
			if (empty($results)) {
				// Check all questions if they've been completed
				$data = array();
				...
				if (isset($_GET['ur'])) {
					...
					$query .= "FROM Questions ";
					$query .= "WHERE question_id IN ( " . $buildCode . "))";
					...
					foreach ($fieldsToCheck as $field) {
						...
						$query = "SELECT " . $field . " FROM Responses WHERE SHA1(record_id) = ?";
						...
						if ($results[0][0] == NULL) {
							...
							$strippedField = preg_replace("/[0-9]/","", $field);
							
							if (!in_array($strippedField, $incompleteQuestions)) {
								$incompleteQuestions[] = $strippedField;
							}
						}
					}
				...
				foreach ($incompleteQuestions as $incompleteSectionId) {
					$data = array();
					$data[] = $incompleteSectionId;
					...
					$incompleteSections[] = $results[0][0];
				}
				
				if ($surveyIncomplete) {
					print "<div class='mustComplete'>";
					print "<p>The survey must be completed before you finish this section. There are remaining questions in:</p>";
					print "<ul>";
					foreach ($incompleteSections as $incSection) {
						print "<li>" . ucfirst(strtolower($incSection)) . "</li>";
					}
					print "</ul>";
					print "</div>";
				} else {
