<?php
	...
	*/
	if (isset($_POST['btnDelete'])) {
		...
		$updateBuildCodeResults = $thisDatabase->update($updateBuildCodeQuery, $data);
	}
	...
	*/
	...
	$query .= "FROM Choices ";
	$query .= "JOIN Questions ON question_id = choice_id ";
	$query .= "JOIN Sections ON section_name = question_section ";
	$query .= "WHERE question_id IN (" . $buildCode . ") ";
	$query .= "ORDER BY section_order, question_id";

	$results = $thisDatabase->select($query);

	$allQuestions = $results;
	
	// moved to menu print "<a href='surveyTool.php?newQ'>Add a question</a>";
	...
	foreach ($allQuestions as $question) {
		...
		$currentSection = preg_replace("/[0-9]/","",$questionName);
		
		if ($currentSection != $previousSection) {
			if ($previousSection != "") {
				print "</div>";
			}
			print "<div class='newSectionBreak' style='border: 1px solid #000; margin: 5em;'>";
			print "<h2 style='text-align: center; margin: 1em;'>" . $question['question_section'];
			print "</h2>";
